...
FInv_SlotAvailabilityResult UInv_InventoryGrid::HasRoomForItem(const FInv_ItemManifest& Manifest, const int32 StackAmountOverride)
{
	...
	//Determine if the item is stackacble
	...
	for (const auto& GridSlot : GridSlots)
	{
		...
		// is the item in grid bround
		if (!IsInGridBounds(GridSlot->GetTileIndex(), GetItemDimensions(Manifest))) continue;

		//can the item fit here ? (1.e. is it out of grid bounds?
											...
											const FGameplayTag& ItemType, const int32 MaxStackSize) const
{
	...
	if(!DoseItemTypeMatch(SubItem, ItemType)) return false;
...
bool UInv_InventoryGrid::DoseItemTypeMatch(const UInv_InventoryItem* SubItem, const FGameplayTag& ItemType) const 
{
...
int32 UInv_InventoryGrid::DetermineFillAmountForSlot(const bool bStackable, const int32 MaxStackSize, const int32 AmounToFill, const UInv_GridSlot* GridSlot) const 
{
	const int32 RoomInSlot = MaxStackSize - GetStackAmount(GridSlot);
	return bStackable ? FMath::Min(AmounToFill, RoomInSlot) : 1;
}
...
void UInv_InventoryGrid::PickUp(UInv_InventoryItem* ClickedInventoryItem, const int32 GridIndex)
{
	...
	// Remove Clicked item form the grid
...
void UInv_InventoryGrid::RemoveItemFromGrid(UInv_InventoryItem* InventoryItem, const int32 GridIndex)
{
	...
	{
		TObjectPtr<UInv_SlottedItem> FoundSlotredItem;
		SlottedItems.RemoveAndCopyValue(GridIndex, FoundSlotredItem);
		FoundSlotredItem->RemoveFromParent();
	}
}
...
UInv_SlottedItem* UInv_InventoryGrid::CreateSlottedItem(UInv_InventoryItem* Item, const bool bStackable, const int32 StackAmount, const FInv_GridFragment* GridFragment, const FInv_ImageFragment* ImageFragment, int32 Index)
{
